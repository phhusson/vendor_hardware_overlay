...
#Keep knownKeys
rm -f tests-win/priorities fail
touch tests-win/priorities tests-win/knownKeys
find . -name AndroidManifest.xml |while read -r manifest;do
	...
	cat tests-win/blacklist |while read -r b;do
	...
	isStatic="$(./tests-win/xml sel -t -m '//overlay' -v @android:isStatic -n "$manifest")"
	[ "$isStatic" != "true" ] && continue

    targetPkg="$(./tests-win/xml sel -t -m '//overlay' -v @android:targetPackage -n "$manifest")"
    [ "$targetPkg" != "android" ] && continue

	#Ensure priorities unique-ness
	priority="$(./tests-win/xml sel -t -m '//overlay' -v @android:priority -n "$manifest")"
	if grep -qE '^'"$priority"'$' tests-win/priorities;then
		fail "$manifest" "priority $priority conflicts with another manifest"
	fi
	echo "$priority" >> tests-win/priorities

	systemPropertyName="$(./tests-win/xml sel -t -m '//overlay' -v @android:requiredSystemPropertyName -n "$manifest")"
	...
	find "$folder" -name \*.xml |while read -r xml;do
		keys="$(./tests-win/xml sel -t -m '//resources/*' -v @name -n "$xml")"
		for key in $keys;do
			grep -qE '^'"$key"'$' tests-win/knownKeys && continue
...
lastpriority="$(sort -n tests-win/priorities |grep -E '^[0-9]{2,3}$' | tail -n 1)"
...
while true;do
    ...
    v=$((v+10))
    if ! grep -qE '\b'$v'\b' tests-win/priorities;then
        echo -e '\tI recommend you use priority' $v
        break
    fi
done
rm -f tests-win/priorities
