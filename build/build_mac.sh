...
echo "$makes" | while read -r f;do
    ...
    path="$(dirname "$f")"
    ./aapt_mac package -f -F "${name}-unsigned.apk" -M "$path/AndroidManifest.xml" -S "$path/res" -I android.jar
    LD_LIBRARY_PATH=./signapk/ java -jar signapk/signapk_mac.jar keys/platform.x509.pem keys/platform.pk8 "${name}-unsigned.apk" "${name}.apk"
